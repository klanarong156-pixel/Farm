...
import {
  Activity, Bell, CalendarDays, ChevronRight, CircleHelp, CloudRain, Cpu, Droplets, Edit3, Gauge, Home as HomeIcon,
...
function DeviceCard({ device, schedule, onName, onMode, onToggle, onSchedule }: {
  ...
  return (
    <article className={`device-card device-card--${meta.tint}`}>
      ...
      <div className="device-card__status"><span className={confirmedOn ? "state-on" : device.status === "OFF" ? "state-off" : "state-unknown"}>{device.status === "unknown" ? "--" : device.status}</span><span>{statusLabel(device)}</span></div>
